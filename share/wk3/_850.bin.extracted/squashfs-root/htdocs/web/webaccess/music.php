...
<!--	<script type="text/javascript" src="fancybox/jquery.mousewheel-3.0.4.pack.js"></script> -->
		...
		<script type="text/javascript">	
			...
			function check_is_ready(){
				var audio_obj = get_by_id("my_audio");
				
				if(audio_obj.readyState == 4){
					audio_obj.play();
				}
			}
			
			function start_play_media(index){
				var audio_obj = get_by_id("my_audio");
				audio_obj.src = media_info.files[index].path;				
				audio_obj.load();
				setTimeout("check_is_ready()", 1000);				
			}
				...
				if (media_info.status == "ok" && media_info.errno == null){
					...
					for (var i = 0; i < media_info.count; i++){
						...
						str += "<tr onMouseOver=\"this.style.background='#D8D8D8'\" onMouseOut=\"this.style.background=''\">"
							 ...
							 //+ "<a id=\"music1\" href=\"/" + obj.path + "\" title=\"" + file_name + "\">"
			...
			function reset_search_box(){
				get_by_id("search_box").value = "<? echo I18N("h", "Search Songs");?>...";	
				show_media_list(false);	
			}
			...
			function load_value(){
				get_by_id("search_box").value = I18N("j", "Search Songs")+"...";
			}
load_lang_obj();
	...
	<body onload="MM_preloadImages('webfile_images/btn_home_.png','webfile_images/x.png');get_login_info();">
		<div id="wrapper">
			<div id="header">
				<div align="right">
				...
				<div id="navi">
					<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
						<tr>
							...
							<td width="82%" class="text_2Copy"><? echo I18N("h", "Music");?></td>
							<td width="10%"></td>
						</tr>
					</table>
				</div>
					...
					<table width="100%" height="49" border="0" border-color:transparent;align="left" cellpadding="0" cellspacing="0">
						<tr>
							...
							<th width="880" class="search2" scope="row">							
								<input type="text" id="search_box" name="search_box" class="search2" value="<? echo I18N("h", "Search Songs");?>..." onfocus="clear_search_box()" onkeyup="show_media_list(true)" size="105" />
							</th>
