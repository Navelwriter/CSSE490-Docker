...
{
	<?
		if(isfile("/etc/services/UPNPC.php")==1) echo 'services: "WEBACCESS,SAMBA,INET.LAN-1,DDNS4.WAN-1,UPNPC",';
		else echo 'services: "WEBACCESS,SAMBA,INET.LAN-1,DDNS4.WAN-1",';
	?>
	OnLoad: function() {},
	OnUnload: function() {}, 
	...
	InitValue: function(xml)
	{
		...
		this.samba_p = PXML.FindModule("SAMBA");
	...
	Initial: function()
	{
		...
		// samba auth status
		this.samba_auth=XG(this.samba_p+"/samba/auth");
		COMM_SetRadioValue("smb_auth", this.samba_auth);
	...
	{		
		...
		XS(this.samba_p+"/samba/auth", COMM_GetRadioValue("smb_auth"));
