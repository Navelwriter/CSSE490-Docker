...
?>
	...
	InitWAN: function ()
	{
		...
		else if ((XG(this.waninetp+"/addrtype") == "ipv6")&& wancable_status==1)
		{
			...
			//+++ Michael Lo, In 6rd case, if assigned prefix len is 64, all IP addresses are assigned to lan.
			if(str_wantype == "6RD")
			{
				if ( str_wanipaddr == "")
				{
					str_wangateway = XG(rwan+"/ipv6/gateway");
				}
				
				//+++ Jerry Kao, when all /64 prefix assigned to LAN, WAN will have no ip address,
				//               thus using wangateway to determine network connected or not.				
				if ((wancable_status==1) && str_wangateway != "")	
				{ 
					wan_network_status=1; 
				}
			}
		...
		else if (is_ppp6 == 1 && wancable_status==1)
		{
			...
			else
			{
				...
				if(XG(wan3inetp + "/ppp6/dns/count")=="1")
				{					
					var str_wanDNSserver  = XG(wan3inetp + "/ppp6/dns/entry:1");
					var str_wanDNSserver2 = XG(rwan4 + "/runtime/inf/inet/ipv6/dns:1");
				}
				else if(XG(wan3inetp + "/ppp6/dns/count")=="2")
				{
					var str_wanDNSserver  = XG(wan3inetp + "/ppp6/dns/entry:1");
					var str_wanDNSserver2 = XG(wan3inetp + "/ppp6/dns/entry:2");				
				}
				else
				{
					// Original
		...
		else if (is_ppp10 == 1 && wancable_status==1)
		{
			...
			//+++ Jerry Kao, modified for show user defined DNS primary in web.												
			if(XG(wan1inetp + "/ppp6/dns/count")=="1")
			{					
				var str_wanDNSserver  = XG(wan1inetp + "/ppp6/dns/entry:1");
				var str_wanDNSserver2 = XG(rwan4 + "/runtime/inf/inet/ipv6/dns:1");
			}
			else if(XG(wan1inetp + "/ppp6/dns/count")=="2")
			{
				var str_wanDNSserver  = XG(wan1inetp + "/ppp6/dns/entry:1");
				var str_wanDNSserver2 = XG(wan1inetp + "/ppp6/dns/entry:2");				
			}
			else
			{
				// Original
		...
		else if(!this.is_ll)
		{
			...
			if(str_wantype == "6RD")
			{				
				OBJ("status").innerHTML  = ((wancable_status==1 && str_wangateway!="") ? str_Connected:str_Disconnected);			
			}
		...
		else
		{
			...
			else if(str_wantype=="AUTO")
			{
				...
				if(rwanmode=="" || rwanmode=="STATEFUL" || enpd=="1")
				{
