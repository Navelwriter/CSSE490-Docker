...
function changes_default_wifi($phyinfuid,$ssid,$password,$mac,$country)
{
	...
	if($country=="CN" && $image_sign != "wrgac05_dlob.hans_dir850l_SN")
	{
		//$ssid = "D-Link_DIR-802";
		$authtype = "OPEN";
		$encrtype = "NONE";
...
if($image_sign == "wrgac05_dlob.hans_dir850l_SN")
{
	...
	{
		//==850 SN 20131220===
		$lan_infp = XNODE_getpathbytarget("", "inf", "uid", $LAN1, 0);
		$lan_inet = get("",$lan_infp."/inet");
		$lan_inetp = XNODE_getpathbytarget("/inet", "entry", "uid", $lan_inet, 0);
		set($lan_inetp."/ipv4/ipaddr","192.168.197.241");	
		set($lan_infp."/dns4","");//dns relay off
		set($lan_infp."/dhcps4","");//disable dhcp server
		//==850 SN 20131220===
		
		$phy = XNODE_getpathbytarget("", "phyinf", "uid", $WLAN1, 0); 
		set($phy."/media/rtsthresh","0"); 	//RTS disabled.
		set($phy."/media/dtim","1");			//DTIM is 1.
		set($phy."/media/beacon","102");		//Beacon int is 102..
		set($phy."/media/dot11n/bw2040coexist",0);//disable 20/40coexist
		$phy1 = XNODE_getpathbytarget("", "phyinf", "uid", $WLAN2, 0); 
		set($phy1."/media/rtsthresh","0"); 	//RTS disabled.
		set($phy1."/media/dtim","1");			//DTIM is 1.
		set($phy1."/media/beacon","102");		//Beacon int is 102..
		set($phy1."/media/dot11n/bw2040coexist",0);//disable 20/40coexist
	}
}
