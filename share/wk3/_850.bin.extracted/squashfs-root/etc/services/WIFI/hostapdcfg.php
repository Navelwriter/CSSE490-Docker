...
//regenerate wifi phyinf runtime table
foreach ("/runtime/phyinf")
{
	...
	if($inf_prefix == $_GLOBALS["UID"])
	{
		$uid_running = 'UID'.cut($uid,1,'.');
		$$uid_running = $uid;//running
	}
	else if($inf_prefix == $_GLOBALS["UID5G"])
	{
		$a = 0;
		$a = cut($uid,1,'.');
		$a = $a + 4;
...
//if the main wifi not running, then clear all guest zone
if($UID1 == "")//2.4g band wlan1
{
	$UID2="";$UID3="";$UID4="";
}
if($UID5 == "")//5g band wlan0
{
	$UID6="";$UID7="";$UID8="";
}
/*	TRACE_debug("idx_running=".$UID1);
	TRACE_debug("idx_running=".$UID2);
	TRACE_debug("idx_running=".$UID3);
	TRACE_debug("idx_running=".$UID4);
	TRACE_debug("idx_running=".$UID5);
	TRACE_debug("idx_running=".$UID6);
	TRACE_debug("idx_running=".$UID7);
	TRACE_debug("idx_running=".$UID8);
*/
$i = 1;//so far check 4 wifi dev each band
$devnum = 0;
while ($i < 9)
{
	$uid = 'UID'.$i;
