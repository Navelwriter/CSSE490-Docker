...
function ddns4generate($inf, $phyinf, $ddnsp,$v6inf,$v6enable,$v6phyinf)
{
	...
	/* Get public IP, DDNS server will use it */
	/*
	$gpip = "wget -q -O - http://icanhazip.com"; // get public IP
	setattr("/tmp/publicip",	"get", $gpip);
	$ipaddr = query("/tmp/publicip");
	
	if($ipaddr == "") //avoid URL invalid
	{
		$gpip = "wget -q -O - http://myexternalip.com/raw";
		setattr("/tmp/publicip",	"get", $gpip);
		$ipaddr = query("/tmp/publicip");
	}
	else $ipaddr = $ipaddr;
	...
	*/
	...
	$set = 'SET '.$provider.' "(NULL)" "'.$username.'" "'.$password.'" "'.$hostname.'" '.$interval; // it should not give WAN IP address to ddnsd.
