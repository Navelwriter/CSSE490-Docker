...
if ($defaultroute!="")
{
	...
	{
		/********************************************************************************************/
		/* Builder modifies this. 2014/08/26                                                        */
		/* Add default route without assigning "default" table.                                     */
		/* It's to fit Realtek HW NAT mechenism to getting MAC address of default GW.               */
		/********************************************************************************************/
		/*echo 'ip route add default via '.$REMOTE.' metric '.$defaultroute.' table default\n';*/
		echo 'ip route add default via '.$REMOTE.' metric '.$defaultroute.'\n';
	}
}
...
if($child!="")
{
	...
	else
	{
		...
		//+++ Jerry Kao, add event for restart WAN-1 when PPPoE Share with IPv4,but without IPv6 on the ppp line.
		echo "event ".$PARAM.".UP\n";
		echo "echo 1 > /var/run/".$PARAM.".UP\n";
	}
}
