...
function commands($inf, $stsp, $phyinf, $dhcpsp)
{
	...
	*/
	...
	if($wanpdvlft == "-1") $wanpdvlft = "0";//avoid radvd crash
	...
	if ($mode == 'STATELESS')
	{
		...
		//UNH-IOL
		//>>>>
		if($onlyula==1 && $isula==1)
		{
			//only have ula address
	...
	else /* STATEFUL */
	{
		...
		$preferlft = query($stsp.'/dhcps6/pd/preferlft');
		$validlft  = query($stsp.'/dhcps6/pd/validlft');
		fwrite(w,$racfg,
			'# radvd config for '.$inf.'\n'.
			'interface '.$ifname.'\n'.
			'{\n'.
		...
		if($isula==0)
		{
			...
			if($preferlft == "" || $preferlft == "-1") $preferlft="3600";
			if($validlft == "" || $validlft == "-1")  $validlft="7200";
